...
describe("ArttacaMarketplaceUpgradeable ERC721 secondary sales", function () {
  ...
  it("Owner receives payment less royalties pct and protocol fee, royalties are paid to splits", async function () {
    ...
    expect((await erc721collection.tokensOfOwner(buyer1.address)).length).to.equal(1);
    expect((await erc721collection.tokensOfOwner(buyer1.address))[0]).to.equal(TOKEN_ID);
    expect(await erc721collection.tokenOfOwnerByIndex(buyer1.address, 0)).to.equal(TOKEN_ID);
